...
# Name of the node-pools for Bhojpur.NET Platform services and applications
SERVICES_POOL="services"
APPLICATIONS_POOL="applications"

K8S_NODE_VM_SIZE=${K8S_NODE_VM_SIZE:="Standard_DS3_v2"}
CERT_NAME="https-certificates"
MYSQL_BHOJPUR_ENCRYPTION_KEY='[{"name":"general","version":1,"primary":true,"material":"7uGh3q8y2DYryJwrVMHs5kWXJlqvHWWt/KJuNi25edI="}]'
...
function install() {
    echo "Bhojpur.NET Platform installer version: $(bhojpur-installer version | jq -r '.version')"
    ...
    if [ "$(az aks show --name ${CLUSTER_NAME} --resource-group ${RESOURCE_GROUP} --query "name == '${CLUSTER_NAME}'" || echo "empty")" == "true" ]; then
      ...
      az aks create \
        ...
        --nodepool-labels bhojpur.net/workload_meta=true bhojpur.net/workload_ide=true \
    ...
    if [ "$(az aks nodepool show --cluster-name ${CLUSTER_NAME} --name ${APPLICATIONS_POOL} --resource-group ${RESOURCE_GROUP} --query "name == '${APPLICATIONS_POOL}'" || echo "empty")" == "true" ]; then
      echo "Node pool ${APPLICATIONS_POOL} exists..."
    else
      echo "Creating ${APPLICATIONS_POOL} node pool..."

      az aks nodepool add \
        --cluster-name "${CLUSTER_NAME}" \
        --enable-cluster-autoscaler \
        --kubernetes-version "${AKS_VERSION}" \
        --labels bhojpur.net/workload_application_services=true bhojpur.net/workload_application_regular=true bhojpur.net/workload_application_headless=true \
        --max-count "50" \
        --max-pods "110" \
        --min-count "3" \
        --name "${APPLICATIONS_POOL}" \
        --node-osdisk-size "100" \
        --node-vm-size "${K8S_NODE_VM_SIZE}" \
        --resource-group "${RESOURCE_GROUP}"
      fi
    ...
    if [ -n "${IMAGE_PULL_SECRET_FILE}" ] && [ -f "${IMAGE_PULL_SECRET_FILE}" ]; then
        if ! kubectl get secret bhojpur-image-pull-secret; then
            kubectl create secret generic bhojpur-image-pull-secret \
    ...
    cat << EOF
==========================
Bhojpur.NET Platform is now installed on your cluster
...
function install_bhojpur() {
  echo "Installing Bhojpur.NET Platform..."

  local CONFIG_FILE="${DIR}/bhojpur-config.yaml"

  bhojpur-installer init > "${CONFIG_FILE}"
  ...
  yq e -i '.application.runtime.containerdRuntimeDir = "/var/lib/containerd/io.containerd.runtime.v2.task/k8s.io"' "${CONFIG_FILE}"

  bhojpur-installer \
    render \
    --config="${CONFIG_FILE}" > bhojpur.yaml
...
function install_jaeger_operator(){
  ...
  kubectl apply -f "${DIR}/charts/assets/jaeger-bhojpur.yaml"
}

function login() {
  echo "Log into Microsoft Azure..."
  az login
...
function setup_mysql_database() {
  MYSQL_BHOJPUR_USERNAME="bhojpur"
  MYSQL_BHOJPUR_PASSWORD=$(openssl rand -base64 20)
  ...
  if [ "$(az mysql server show --name ${MYSQL_INSTANCE_NAME} --resource-group ${RESOURCE_GROUP} --query "name == '${MYSQL_INSTANCE_NAME}'" || echo "empty")" == "true" ]; then
    ...
    az mysql server update \
      --admin-password "${MYSQL_BHOJPUR_PASSWORD}" \
      --name "${MYSQL_INSTANCE_NAME}" \
      --resource-group "${RESOURCE_GROUP}"
  else
    echo "Creating MySQL instance..."
    az mysql server create \
      --admin-user "${MYSQL_BHOJPUR_USERNAME}" \
      --admin-password "${MYSQL_BHOJPUR_PASSWORD}" \
      --auto-grow Enabled \
      --location "${LOCATION}" \
      --name "${MYSQL_INSTANCE_NAME}" \
  ...
  if [ "$(az mysql db show --name ${DB_NAME} --resource-group ${RESOURCE_GROUP} --server-name ${MYSQL_INSTANCE_NAME} --query "name == '${DB_NAME}'" || echo "empty")" == "true" ]; then
    echo "Bhojpur MySQL database exists..."
  else
    echo "Creating Bhojpur MySQL database..."
    az mysql db create \
      --name "${DB_NAME}" \
      --resource-group "${RESOURCE_GROUP}" \
  ...
  kubectl create secret generic "${SECRET_DATABASE}" \
    --from-literal=encryptionKeys="${MYSQL_BHOJPUR_ENCRYPTION_KEY}" \
    --from-literal=host="${MYSQL_INSTANCE_NAME}.mysql.database.azure.com" \
    --from-literal=password="${MYSQL_BHOJPUR_PASSWORD}" \
    --from-literal=port="3306" \
    --from-literal=username="${MYSQL_BHOJPUR_USERNAME}@${MYSQL_INSTANCE_NAME}" \
    --dry-run=client -o yaml | \
    kubectl replace --force -f -
}
...
function uninstall() {
  ...
  read -p "Are you sure you want to delete: Bhojpur.NET Platform (y/n)? " -n 1 -r
  if [[ $REPLY =~ ^[Yy]$ ]]; then
    set +e
    ...
    kubectl delete secret bhojpur-image-pull-secret
